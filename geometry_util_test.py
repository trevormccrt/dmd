...
def test_minimum_periodic_dist():
    ...
    with torch.no_grad():
        min_dist, p2_remap = geometry_util.minimum_periodic_distance(
            test_1d_1, test_1d_2)
    np.testing.assert_allclose(np.squeeze(p2_remap), [2 * np.pi + 0.5, 1.2], rtol=1e-6)
    ...
    with torch.no_grad():
        min_dist, p2_remap = geometry_util.minimum_periodic_distance(
            test_2d_1, test_2d_2)
    np.testing.assert_allclose(p2_remap, [[-0.5, -0.5], [1, 1]])
...
def test_closest_points_periodic():
    ...
    with torch.no_grad():
        min_dist,  matched, matches = geometry_util.closest_points_periodic(points)
        matched_ring = geometry_util.torch_angles_to_ring(matched)
    ...
    np.testing.assert_allclose(matched_ring, matches_ring, atol=1e-6)
